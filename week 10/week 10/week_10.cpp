...
{
	Image[(Y * W + X) * 3] = B; // Blue 성분
	Image[(Y * W + X) * 3 + 1] = G; // Green 성분
	Image[(Y * W + X) * 3 + 2] = R; // Red 성분
}
...
int main()
{
	...
	for (int a = 0; a < 120; a++) {
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[(a * W + i) * 3] = (BYTE)(255 * (1.0 - wt)); // Blue
			Image[(a * W + i) * 3 + 1] = (BYTE)(255 * (1.0 - wt)); // Green
			Image[(a * W + i) * 3 + 2] = (BYTE)(255 * wt); // Red
		}
	}
	...
	SaveBMPFile(hf, hInfo, hRGB, Image, hInfo.biWidth, hInfo.biHeight, "fc_sample_gradation.bmp");
